...
```{r}
raw_new <- read.csv("Data/Fish_Comp_Values_3D.csv")
raw_old <- read.csv("Data/older_Fish_Comp_Values_3D.csv") %>% mutate(Angle = deg2rad(Angle),
                                                                     Fish1_Yaw_Heading = deg2rad(Fish1_Yaw_Heading),
                                                                     Fish1_Yaw_Heading_PS = deg2rad(Fish1_Yaw_Heading_PS))

hist(raw_new$Angle - deg2rad(raw_old$Angle))
hist(raw_new$Fish1_Yaw_Heading - deg2rad(raw_old$Fish1_Yaw_Heading))
hist(raw_new$Fish1_Yaw_Heading_PS - deg2rad(raw_old$Fish1_Yaw_Heading_PS))
...
nn_accel_sum <- fish_data_nn_normal %>% mutate(distance_bins = round_any(Relative_X,0.25)) %>%
...
tester_model <- lm(Fish1_Yaw_Heading_PS ~ sin(Angle):Ablation:Darkness, data = fish_data_nn_model) 
Anova(tester_model)
...
ggplot(nn_heading_sum %>% filter(Flow == "Still Water"),
  ...
  geom_line(aes(y = rad2deg(predict(tester_model,
...
# emmeans(tester_model, ~ Ablation|Darkness, type = "response")
# emmeans(tester_model, ~ Darkness|Ablation, type = "response")
