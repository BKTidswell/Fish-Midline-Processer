...
perp_angle_df <- fish_comp_data %>% filter(Angle >= deg2rad(75) & Angle <= deg2rad(105)) %>% filter(Distance >= 2 & Distance <= 100) %>% filter(Flow == "Still Water")
...
ggplot(perp_angle_df, aes(x = Flow_Ablation_Darkness, y = Fish1_Yaw_Heading_PS,
                          fill = Flow_Ablation_Darkness, color = Flow_Ablation_Darkness))+
  ...
  scale_color_manual(values = c(light_no_ab_color,light_ab_color,dark_no_ab_color,dark_ab_color,"red")) +
  theme_classic()+
  ggtitle("Heading per Second, 2 to 100 BL, 75 to 100 Degrees")
