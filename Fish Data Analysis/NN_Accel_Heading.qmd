...
nn_heading_sum <- fish_data_nn_normal %>% mutate(angle_bins = round_any(Angle,pi/5)) %>%
...
heading_ps_angle_step <- stepAIC(heading_ps_angle_base,
                                   list(upper = ~ (sin(Angle)+Ablation:sin(Angle)+Darkness:sin(Angle)), lower = ~1),
                                   direction = "both",
                                   trace = F)
...
ggplot(fish_data_nn %>% filter(Flow == "Still Water"),
       aes(x = Angle, y = Fish1_Yaw_Heading_PS, color = interaction(Ablation, Darkness, sep =", ")))+
