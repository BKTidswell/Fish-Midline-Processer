...
fish_comp_data_switch_fish <- fish_comp_data %>% mutate(Fish3 = Fish1,
                                       ...
                                       mutate(Angle = -1*ifelse(Angle >= 0, Angle - pi, Angle + pi),
...
fish_data_nn <- fish_comp_data_combo %>% group_by(Fish1,Tailbeat_Num,Data_ID) %>%
                                    ...
                                    #filter(row_n == 1) %>%
                                    ungroup() %>%
                                    mutate(Fish1_Yaw_Heading_PS = Fish1_Yaw_Heading_PS) %>%
...
manual_model <- lm(Fish1_Yaw_Heading_PS ~ sin(Angle) + sin(Angle):Ablation + sin(Angle):Darkness + sin(Angle):Ablation:Darkness, 
                   data = fish_data_nn_model) 
Anova(manual_model)

manual_model_2 <- lm(Fish2_Yaw_Heading_PS ~ sin(Angle) + sin(Angle):Ablation + sin(Angle):Darkness + sin(Angle):Ablation:Darkness, 
                   data = fish_data_nn_model %>% mutate(Angle = ifelse(Angle >= 0, Angle - pi, Angle + pi))) 
Anova(manual_model_2)
...
ggplot(nn_heading_sum %>% filter(Flow == "Still Water"),
  ...
  scale_x_continuous(breaks = unique(nn_heading_sum$angle_bins),
                     labels = rad2deg(unique(nn_heading_sum$angle_bins))) +
  labs(color = "Sensory Conditions")+
  ggtitle(paste0("All Fish From ",min_dist," to ",max_dist))
