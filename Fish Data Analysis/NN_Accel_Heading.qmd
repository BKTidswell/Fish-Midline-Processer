...
max_x <- ceiling(max(fish_comp_data$Fish1_X, na.rm = T)) - 1.5
min_x <- floor(min(fish_comp_data$Fish1_X, na.rm = T)) - 1.5
max_y <- ceiling(max(fish_comp_data$Fish1_Y, na.rm = T)) - 0.75
min_y <- floor(max(fish_comp_data$Fish1_Y, na.rm = T)) - 0.75
...
Distribution of turning velocities at say 90 degrees just make a Kolmogorov--Smirnov to compare distribution. See if distribution is different Not larger or smaller? Also maybe just do a t test trim the edge points
...
perp_angle_df <- fish_data_nn_normal %>% filter(Mean_Angle >= deg2rad(75) & Mean_Angle <= deg2rad(105)) %>% filter(Flow == "Still Water")#%>% filter(Distance >= 0 & Distance <= 3) %>% filter(Flow == "Still Water")

perp_angle_df_neg <- fish_data_nn_normal %>% filter(Mean_Angle >= deg2rad(-105) & Mean_Angle <= deg2rad(-75)) %>% filter(Flow == "Still Water") %>% mutate(Fish1_Yaw_Heading_PS = -1*Fish1_Yaw_Heading_PS)
