...
mean_group_still <- ggplot(school_data %>% filter(Flow == "Still Water"),
       aes(x = interaction(Ablation,Darkness,sep=", "), y = Mean_Groups,
           color = interaction(Ablation,Darkness,sep=", "),
           fill = interaction(Ablation,Darkness,sep=", ")))+
  stat_dots(side = "both", binwidth = unit(dot_binwidth, "npc")) +
  geom_point(aes(y = Mean_Groups_Mean_Val), size = 5, color = "Black") +
  geom_errorbar(aes(ymin=Mean_Groups_Mean_Val-Mean_Groups_SD_Val, ymax=Mean_Groups_Mean_Val+Mean_Groups_SD_Val), width=.2, color = "Black")+
  guides(fill = guide_legend(title = "Condition")) +
  scale_fill_manual(values=c(light_no_ab_color, light_ab_color, dark_no_ab_color)) +
  scale_color_manual(values=c(light_no_ab_color, light_ab_color, dark_no_ab_color)) +
  #ggtitle("Effect of Available Senses on Speed (BL/s) in Still Water") +
  xlab("") +
  ylab("Mean Number of Groups") +
  theme_light()+ 
  theme(legend.position = "none") +
  #ylim(0,8.5)+
  stat_compare_means(comparisons = my_comparisons, label = "p.signif", hide.ns = TRUE) +
...
mean_group_flow <- ggplot(school_data %>% filter(Flow == "Flowing Water (2 BL/s)"),
       aes(x = interaction(Ablation,Darkness,sep=", "), y = Mean_Groups,
           color = interaction(Ablation,Darkness,sep=", "),
           fill = interaction(Ablation,Darkness,sep=", ")))+
  stat_dots(side = "both", binwidth = unit(dot_binwidth, "npc")) +
...
group_still <- ggplot(group_sum_data %>% filter(Flow == "Still Water"),
    ...
    theme_light()+ 
    scale_fill_manual(values=c("#1D3557","#325A7C","#457B9D","#7BAFBF","#A8DADC","#D0EBE6","#F1FAEE","#e63946")) +
    theme(axis.text.x = element_text(size = 15),
          axis.text.y = element_text(size = 15),
          axis.title.y = element_text(size = 15))
...
groups_glm <- glm(Groups ~ Ablation:Darkness:Flow, data = school_data)
emmeans(area_glm, specs = pairwise ~ Ablation:Darkness | Flow)
