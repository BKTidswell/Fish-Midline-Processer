...
eight_fish_still_model <- ggplot(eight_fish_turning_data %>% filter(Flow == "Still Water"),
...
only_flow_model <- glm(turn_bool ~ Fish_Num_Ratio * Ablation * Darkness, 
                 data = eight_fish_turning_data %>% filter(Flow == "Flowing Water (2 BL/s)"), 
                 family = binomial)

eight_fish_flow_model <- ggplot(eight_fish_turning_data %>% filter(Flow == "Flowing Water (2 BL/s)"),
                           aes(x = Fish_Num_Ratio, y = turn_bool, color = Flow_Ablation_Darkness))+
  #geom_jitter(alpha = 0.1, height = 0.025)+
