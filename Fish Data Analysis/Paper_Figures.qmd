...
fish_comp_data_ring_angles <- fish_comp_data %>% filter(Distance >= min_dist & Distance <= max_dist) %>%
                                                 ...
                                                 #          Flow,Ablation,Darkness,Dist_Cat,Flow_Ablation_Darkness,Fish1,Tailbeat_Num) %>%
...
perp_angle_df <- fish_data_nn_normal %>% filter(Mean_Angle >= deg2rad(75) & Mean_Angle <= deg2rad(105))

perp_angle_df_neg <- fish_data_nn_normal %>% filter(Mean_Angle >= deg2rad(-105) & Mean_Angle <= deg2rad(-75)) %>% 
