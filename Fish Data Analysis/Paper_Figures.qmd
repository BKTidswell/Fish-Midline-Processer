...
abalated_dark_turning_data <- read.csv("Data/ablated_dark_data_turning.csv") %>%
...
eight_fish_turning_data <- rbind(annushka_turning_data,abalated_dark_turning_data)
...
single_fish_data_sum  <- single_fish_turning_data %>% na.omit %>%
                                     group_by(Fish_Ratio,Fish_Num_Ratio,Flow,Ablation,Darkness,Flow_Ablation_Darkness,Type) %>%
                                     summarize(mean_right_turn = mean(turn_bool),
                                               sd_right_turn = sd(turn_bool),
                                               se_right_turn = sd_right_turn/sqrt(n()),
                                               n = n()) %>%
                                     ungroup() %>%
                                     filter(Flow == "Still Water" & Darkness == "Light" & Ablation == "No Ablation")
...
```{r}
placeholder_tibble <- tibble("Fish_Ratio" = c("0:7"),
                             "Fish_Num_Ratio" = c(0.875),
                             "Flow" = c("Still Water"),
                             "Ablation" = c("No Ablation"),
                             "Darkness" = c("Light"),
                             "Type" = c("Combined Single"),
                             "n" = c(0))

light_no_ab_eight_single_graph_data <- eight_fish_data_sum %>% 
                                      filter(Flow == "Still Water" & Darkness == "Light" & Ablation == "No Ablation") %>%
                                      full_join(.,single_fish_data_sum) %>%
                                      full_join(.,placeholder_tibble)

light_ab_eight_single_graph_data <- eight_fish_data_sum %>% 
                                      filter(Flow == "Still Water" & Darkness == "Light" & Ablation == "Ablated") %>%
                                      full_join(.,single_fish_data_sum) %>%
                                      full_join(.,placeholder_tibble)

dark_no_ab_eight_single_graph_data <- eight_fish_data_sum %>% 
                                      filter(Flow == "Still Water" & Darkness == "Dark" & Ablation == "No Ablation") %>%
                                      full_join(.,single_fish_data_sum) %>%
                                      full_join(.,placeholder_tibble)

dark_ab_eight_single_graph_data <- eight_fish_data_sum %>% 
                                      filter(Flow == "Still Water" & Darkness == "Dark" & Ablation == "Ablated") %>%
                                      full_join(.,single_fish_data_sum) %>%
                                      full_join(.,placeholder_tibble)


light_eight_and_single_graph <- ggplot(light_no_ab_eight_single_graph_data,
...
ablation_eight_and_single_graph <- ggplot(light_ab_eight_single_graph_data,
       aes(x = Fish_Num_Ratio, y = mean_right_turn, color = Type))+
  geom_point()+
...
dark_eight_and_single_graph <- ggplot(dark_no_ab_eight_single_graph_data,
       aes(x = Fish_Num_Ratio, y = mean_right_turn, color = Type))+
  geom_point()+
  geom_col(aes(x = Fish_Num_Ratio, y = n/80, fill = Type), color = NA, alpha = 0.3, position = "dodge2")+
  geom_smooth(se = F, method = glm, formula = y ~ x, method.args = list(family = "binomial"))+
...
dark_ablation_eight_and_single_graph <- ggplot(dark_ab_eight_single_graph_data,
       aes(x = Fish_Num_Ratio, y = mean_right_turn, color = Type))+
  geom_point()+
  geom_col(aes(x = Fish_Num_Ratio, y = n/80, fill = Type), color = NA, alpha = 0.3, position = "dodge2")+
  geom_smooth(se = F, method = glm, formula = y ~ x, method.args = list(family = "binomial"))+
  ...
  xlab("# Fish on Left : # Fish on Right") +
  ylab("Probability of a Right Turn") +
  scale_colour_manual(values = c(sim_color,dark_ab_color), labels=function(x){sub("\\s", "\n", x)}) +
  scale_fill_manual(values = c(sim_color,dark_ab_color), labels=function(x){sub("\\s", "\n", x)}) +
  #ggtitle("Observed Probability of a Right Turn Based on Ratio of Fish to the Left and Right") +
  scale_x_continuous(breaks = c(0,0.125,0.25,0.375,
                                0.5,0.625,0.75,0.875),
                     labels = c("7:0","6:1","5:2","4:3",
...
dark_ablation_eight_and_single_graph
```
...
turning_single_test_plot <- ggarrange(
          light_eight_and_single_graph, ablation_eight_and_single_graph, 
          dark_eight_and_single_graph, dark_ablation_eight_and_single_graph,
          labels = c("A", "B", "C", "D"),
          ncol = 2, nrow = 2)
