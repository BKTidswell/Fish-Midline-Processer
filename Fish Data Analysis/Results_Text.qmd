...
groups_lmer_still <- glmer(Groups ~ Ablation*Darkness + (1|Date), data = school_data %>% filter(Flow == "Still Water"),
                           family = poisson)
...
groups_lmer_flow <- glmer(Groups ~ Ablation*Darkness + (1|Date), data = school_data %>% filter(Flow == "Flowing Water (2 BL/s)"),
                           family = poisson)
...
heading_PS_model_close <- lmer(Fish1_Yaw_Heading_PS ~ sin(Angle) + sin(Angle):Ablation + sin(Angle):Darkness + sin(Angle):Ablation:Darkness + (1|Date), 
                   data = fish_data_nn_normal_close %>% filter(Flow == "Still Water")) 
aov_heading_PS_model_close <- Anova(heading_PS_model_close)

plot(heading_PS_model_close)
qqnorm(resid(heading_PS_model_close))
qqline(resid(heading_PS_model_close))

test(emmeans(heading_PS_model_close, pairwise ~Ablation*Darkness | Angle, at = list(Angle = c(pi/2))))


heading_PS_model_far <- lmer(Fish1_Yaw_Heading_PS ~ sin(Angle) + sin(Angle):Ablation + sin(Angle):Darkness + sin(Angle):Ablation:Darkness + (1|Date) , 
                   data = fish_data_nn_normal_far %>% filter(Flow == "Still Water")) 
aov_heading_PS_model_far <- Anova(heading_PS_model_far)

plot(heading_PS_model_far)
qqnorm(resid(heading_PS_model_far))
qqline(resid(heading_PS_model_far))
