...
countour_lag_data <- raw_heading_corrs_graphing %>% mutate(round_corrs = round_any(corrs, 0.25),
                                                           round_dists = round_any(Distance, 2))

ggplot(countour_lag_data %>% filter(Flow == "Still Water" & Darkness == "Dark"),
       aes(x = Distance, y = corrs, color = lags))+
  geom_point()+
  scale_color_viridis()+
  theme_classic()
