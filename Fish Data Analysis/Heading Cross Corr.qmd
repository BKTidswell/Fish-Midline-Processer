...
windowed_correlation_finder <- function(x,y){
  ...
  out_corrs <- out_df$max_corr
  out_lags <- out_df$max_corr_lag

  length(out_corrs) <- length(x)
  length(out_lags) <- length(x)
...
Okay so now a specific correaltion version
...
raw_heading_corrs <- raw_fish_comp_data_combo %>% #filter(Data_ID == "2021_8_3_11" & Fish1 == "individual8"  & Fish2 == "individual7") %>%
                                          ...
                                          filter(len_run > 2) %>%
                                          group_by(Year,Month,Day,Trial,Fish1,Fish2) %>%
                                          mutate(corrs = get_corrs_lags_ben(Fish1_Yaw_Heading,Fish2_Yaw_Heading)[[1]],
                                                 lags = get_corrs_lags_ben(Fish1_Yaw_Heading,Fish2_Yaw_Heading)[[2]])
...
ggplot(raw_heading_corrs_sum %>% filter(Flow == "Still Water"),
       aes(x = interaction(Ablation,Darkness,sep=", "), y = max_corr,
           color = interaction(Ablation,Darkness,sep=", "),
           fill = interaction(Ablation,Darkness,sep=", ")))+
  stat_dots(side = "both", binwidth = unit(dot_binwidth/2, "npc")) +
...
max_corr_glmer <- lmer(max_corr ~ Ablation + Darkness + (1|Data_ID), data = raw_heading_corrs_sum %>% filter(Flow == "Still Water"))
Anova(max_corr_glmer)


max_corr_vs_dist_glmer <- lmer(max_corr_distance ~ max_corr*(Ablation + Darkness) + (1|Data_ID), data = raw_heading_corrs_sum %>% filter(Flow == "Still Water"))
Anova(max_corr_vs_dist_glmer)
