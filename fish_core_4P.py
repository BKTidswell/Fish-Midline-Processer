...
def generate_midline(one_fish,time_points):
	...
	fish_h_x = one_fish["head"]["x"]
	fish_h_y = one_fish["head"]["y"]

	fish_m2_x = one_fish["midline2"]["x"]
	fish_m2_y = one_fish["midline2"]["y"]

	#Predict the path of that midline 2 and head points
	predict_fish_h = splprep_predict(fish_h_x,fish_h_y,time_points)
	predict_fish_m2 = splprep_predict(fish_m2_x,fish_m2_y,time_points)
	...
	for i in range(time_points-1):
		...
		for j in range(len(b_parts)):
			...
			h_point = [ predict_fish_h[0][i],predict_fish_h[1][i] ]
			m2_point = [ predict_fish_m2[0][i],predict_fish_m2[1][i] ]
			
			#And pass them through get_para_perp_dist to get the points out
			temp_para[j],temp_perp[j] = get_para_perp_dist(current_fish,h_point,m2_point)

		#Append those to the larger array
		para_a.append(temp_para)
		perp_a.append(temp_perp)

		body_line[i][0][0] = fish_h_x[i]
		body_line[i][0][1] = fish_m2_x[i]

		body_line[i][1][0] = fish_h_y[i]
		body_line[i][1][1] = fish_m2_y[i]
